...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $user_image_name = $_FILES['user_image']['name']; // Get the name of the uploaded image file
    ...
    $user_image_path = $upload_directory . basename($user_image_name);
    ...
    if (move_uploaded_file($_FILES["user_image"]["tmp_name"], $user_image_path)) {
        ...
        $stmt = $conn->prepare("INSERT INTO userCred (userName, userPassword, userImage) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $username, $hashed_password, $user_image_name); // Bind user image name to the statement
        $stmt->execute();

        // Check if user was successfully inserted
        if ($stmt->affected_rows > 0) {
            // Registration successful, redirect user to login page
            header("Location: http://localhost/Projects/Logins/userLogin.html");
            exit();
        } else {
